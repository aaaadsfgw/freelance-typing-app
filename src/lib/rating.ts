...
export function commentFor(
  ...
  if (fast && precise) {
    return combo
      ? "高速かつ正確。コンボも安定していて、クライアントからの指名が止まりません。"
      : "高速かつ正確。クライアントからの指名が止まりません。";
  }
  if (fast && sloppy) {
    return "処理速度は一級品。ミスを減らせばさらに売上を伸ばせそうです。";
  }
  if (slow && precise) {
    return "丁寧で安定した仕事ぶり。次は返信速度を上げてみましょう。";
  }
  if (slow && sloppy) {
    return "まずは正確な入力を意識して、安定して案件をこなしていきましょう。";
  }
  if (fast) {
  ...
  if (precise) {
    return busy
      ? "丁寧に案件を回せてます。もう少し返信を早めると、売上の伸びしろがあります。"
      : "正確な仕事は信頼されます。もう少し返信を早めてみましょう。";
  }
  if (sloppy) {
    return "処理は進んでいます。ミスを減らすと、評価も報酬も一気に安定します。";
  }
  return "速度と正確性のバランスを整えれば、もっと安定して稼げるはずです。";
}
